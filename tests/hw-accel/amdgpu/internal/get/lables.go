// The content of this file is taken from 'tests/hw-accel/nvidiagpu/internal/nvidiagpuconfig/config.go'. The full
...
func LabelPresentOnAtLeastOneNode(apiClient *clients.Settings, nodeLabel string, nodeSelector map[string]string) (bool, error) {
	nodeBuilder, err := nodes.List(apiClient, metav1.ListOptions{LabelSelector: labels.Set(nodeSelector).String()})
